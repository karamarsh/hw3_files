...
#ggplot(order_by(TotalPop, -Region, pop_by_region), aes(x = Region, y = TotalPop, fill = Region)) +
                 #geom_bar(stat = "identity", position = "dodge") +
                 #scale_fill_brewer(palette = "Set2") + scale_y_continuous(labels = scales::comma) + coord_flip()

ggplot(pop_by_region, aes(x = reorder(TotalPop, -Region), y = TotalPop, fill = Region)) + 
...
jobtype <- data.frame(
  name = c("Construction", "Office", "Production", "Professional", "Service"),
  value = c(27, 26, 28, 24, 25)
 )

jobtype <- melt(county_MI_x, id.vars = "Species")
df[1:5,]
